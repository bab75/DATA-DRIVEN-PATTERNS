...
# Simplified CSS to avoid TypeError
st.markdown("""
    <style>
...
1. **Fix TypeError**:
   - Simplify CSS to minimize HTML injection issues, ensuring compatibility with Streamlit Cloud and various Streamlit versions.
   - Use `st.markdown` with `unsafe_html=True` carefully, avoiding complex selectors that might cause issues.

2. **PDF Generation**:
   - Continue using `reportlab` for portable, professional PDF reports, avoiding LaTeX dependencies.
   - Ensure PDFs include headers, sections, and fundamentals tables.

3. **Remove Sentiment Analysis**:
   - Confirm no `vaderSentiment` or X API dependencies, as previously removed.

4. **Core Features**:
   - **Real-Time Data**: Fetch Open, High, Low, Close, Volume, and fundamentals (EPS, P/E, PEG, P/B, ROE, Revenue, Debt/Equity) via `yfinance`.
   - **Manual Inputs**: Sidebar form for fundamentals, defaulting to `None`.
   - **CSV Upload**: Process technical indicators, analysis works without fundamentals.
   - **Price Display**: Show stock data as of June 26, 2025, 4:20 PM EDT.
   - **Reports**: Quick Scan, Moderate Detail, In-Depth Analysis, Visual Summary, Interactive Dashboard.
   - **Exports**: Markdown, PDF, CSV downloads.

5. **Enhancements**:
   - **ADX Indicator**: Via `ta` library for trend strength.
   - **Historical Trends**: Detect bullish/bearish patterns.
   - **Error Handling**: Robust checks for tickers, CSV, and `yfinance` failures.
   - **UI**: Simplified, responsive Tailwind-like styling.

### Dependencies
```bash
pip install streamlit pandas yfinance plotly ta reportlab
```

### Updated Streamlit App Code
The CSS is simplified, and the code is optimized for Streamlit Cloud compatibility.

<xaiArtifact artifact_id="c322842e-9883-4724-a3a8-8b7626d1d0b8" artifact_version_id="f224c0b3-3ecb-4928-8d91-78b0fa3c429c" title="stock_analyzer_app.py" contentType="text/python">
import streamlit as st
import pandas as pd
import yfinance as yf
import plotly.express as px
