...
def calculate_profits(data, strategies, strategy_variant, start_date, end_date):
    if data is None:
        st.error("No data available for profit calculations. Please check the ticker or date range.")
        return None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None, None
    if not isinstance(data, pd.DataFrame) or data.empty or not all(col in data.columns for col in ['Open', 'High', 'Low', 'Close']):
