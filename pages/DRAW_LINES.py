...
def load_data(primary_file, data_source, symbol, start_date, end_date):
        ...
        try:
            ...
            for col in numeric_cols:
                ...
                if nulls > 0 or infs > 0:
                    ...
                    validation_issues.append(f"Column '{col}': {nulls} null values (rows: {null_rows[:5] + ['...'] if null_rows else []}), {infs} inf values (rows: {inf_rows[:5] + ['...'] if inf_rows else []})")
    ...
    if not aapl_df.empty:
        try:
            ...
            aapl_df['stochastic_k'] = ta.momentum.StochasticOscillator(aapl_df['high'], aapl_df['low'], aapl_df['close'], window=14, smooth_window=3).stoch()
            aapl_df['stochastic_d'] = ta.momentum.StochasticOscillator(aapl_df['high'], aapl_df['low'], aapl_df['close'], window=14, smooth_window=3).stoch_signal()
            aapl_df['adx'] = ta.trend.ADXIndicator(aapl_df['high'], aapl_df['low'], aapl_df['close'], window=14).adx()
            aapl_df['atr'] = ta.volatility.AverageTrueRange(aapl_df['high'], aapl_df['low'], aapl_df['close'], window=14).average_true_range()
            ichimoku = ta.trend.IchimokuIndicator(aapl_df['high'], aapl_df['low'], window1=9, window2=26, window3=52)
            ...
            aapl_df['std_dev'] = aapl_df['close'].rolling(window=20).std()
            aapl_df['rvol'] = aapl_df['volume'] / aapl_df['volume'].rolling(window=20).mean()
            
            # Fibonacci retracement levels
            recent_high = aapl_df['high'].rolling(window=20).max()
            recent_low = aapl_df['low'].rolling(window=20).min()
            ...
            indicator_cols = ['rsi', 'macd', 'signal', 'stochastic_k', 'stochastic_d', 'adx', 'atr', 'senkou_span_a', 'senkou_span_b', 'ma20', 'std_dev', 'rvol', 'fib_236', 'fib_382', 'fib_50', 'fib_618']
            initial_len = len(aapl_df)
            ...
            st.error(f"Error computing technical indicators: {str(e)}. Please ensure sufficient data points.")
            return pd.DataFrame(), pd.DataFrame()
...
def add_candlestick_trace(fig, df, row):
    hover_texts = [
        f"Date: {row['date'].strftime('%m-%d-%Y')}<br>Open: ${row['open']:.2f}<br>High: ${row['high']:.2f}<br>Low: ${row['low']:.2f}<br>Close: ${row['close']:.2f}<br>Volume: {row['volume']:,.0f}<br>RSI: {row['rsi']:.2f}<br>RVOL: {row['rvol']:.2f}"
    ...
    for _, row in buy_signals.iterrows():
        fig.add_annotation(x=row['date'], y=row['high'], text="Buy", showarrow=True, arrowhead=2, ax=0, ay=-30, font=dict(color="#000000"), row=row, col=1)
...
<body>
    ...
    <div class="section">
        ...
        <div class="metric-box">
            <p><b>Win Rate:</b> {backtest_results['Win Rate']:.2f}%</p>
            <p><b>Profit Factor:</b> {backtest_results['Profit Factor']:.2f}</p>
            <p><b>Total Return:</b> {backtest_results['Total Return']:.2f}%</p>
            <p><b>Trades:</b> {backtest_results['Trades']}</p>
        </div>
    </div>
