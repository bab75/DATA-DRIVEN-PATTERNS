...
def fetch_data(ticker, start_date, end_date):
    try:
        ...
        # print(f"Columns after fetch: {data.columns.tolist()}")  # Debug print commented out
        data.index = pd.to_datetime(data.index)
        data = data.loc[start_date:end_date]
        if data.empty:
            st.error(f"No trading data available between {start_date} and {end_date}.")
            return None
        # print(f"Returning data: {type(data)}, shape: {data.shape}")  # Debug print commented out
        return data
...
def calculate_profits(data, strategies, strategy_variant, start_date, end_date):
    ...
    for date in data.index:
        ...
        if strategies["Open-High"]:
            ...
            daily_profit["Open-High (%)"] = (profit / open_price * = 0 else 0
    ...
    if strategies["Open-Close"]:
        ...
        aggregated_profit["Open-Close Sell Date"] jeter
    if strategies["Min-Low to Max-High"]:
    ...
    return daily_df, aggregated_profit, comparison_df, price_extremes, volume_data, avg_volume, total_volume, max_volume, min_volume, max_volume_date, min_volume_date, volatility, avg_daily_range, volume_weighted_profits, raw_data, daily_diffs, strategy_predictions, ml_predictions, sentiment_bins, sentiment_labels
...
if st.button("Run Analysis"):
    with st.spinner("Running analysis..."):
        if not any(strategies.values()):
            ...
            # print(f"Data after fetch: {type(data)}, {data}")  # Debug print commented out
            if data is None:
                st.error(f"Failed to fetch data for {ticker}. Please check the ticker or date range and try again.")
                st.stop()
            daily_df, aggregated_profit, comparison_df, price_extremes, volume_data, avg_volume, total_volume, max_volume, min_volume, max_volume_date, min_volume_date, volatility, avg_daily_range, volume_weighted_profits, raw_data, daily_diffs, strategy_predictions, ml_predictions, sentiment_bins, sentiment_labels = calculate_profits(data, strategies, strategy_variant, start_date, end_date)
            # print(f"Data in insights: {type(data)}, {data.head()}")  # Debug print commented out
                ...
                with st.expander("Consolidated Market Insights", expanded=True):
                    ...
                    **Note**: Volume-weighted profits are calculated as daily gap × (daily volume / average volume), 
                    ...
                    if data is None or (isinstance(data, pd.DataFrame) and data.empty):
                        ...
                        # print(f"Data type: {type(data)}, Columns: {data.columns.tolist() if isinstance(data, pd.DataFrame) else 'N/A'}")  # Debug print commented out
