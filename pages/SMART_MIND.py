...
if 'profit_threshold' not in st.session_state:
    st.session_state.profit_threshold = 1.0
...
if submit_button:
    if st.session_state.symbol and st.session_state.start_date and st.session_state.end_date:
        if st.session_state.start_date > st.session_state.end_date:
        ...
        elif st.session_state.end_date > datetime.now().date():
            ...
            try:
                ...
                if not df.empty:
                    ...
                    filtered_df = df[df["Profit_Percent"] >= st.session_state.profit_threshold]
                    st.dataframe(filtered_df, use_container_width=True)
                    st.write(f"Showing {len(filtered_df)} days with Profit % >= {st.session_state.profit_threshold}%")
                    ...
                    # --- Price Trend Chart (Interactive Candlestick) ---
                    ...
                    try:
                        if all(col in df.columns for col in ["Date", "Open", "High", "Low", "Close"]):
                            ...
                            fig.update_layout(title=f"{st.session_state.symbol} Candlestick Chart", xaxis_title="Date", yaxis_title="Price")
