...
# Initialize session state to preserve inputs across refreshes
...
with st.form(key='stock_form'):
    # Input fields for stock symbol, start date, end date, and comparison metric
    ...
    st.session_state.comparison = st.selectbox(
    ...
    if st.session_state.symbol and st.session_state.start_date and st.session_state.end_date:
        if st.session_state.start_date > st.session_state.end_date:
            ...
            try:
                ...
                data = yf.download(st.session_state.symbol, start=st.session_state.start_date, end=st.session_state.end_date, auto_adjust=False)
                ...
                if not data.empty:
                    ...
                    # Option to download raw data as CSV
                    csv = data.to_csv()
                    st.download_button(
                        label="Download Raw Data as CSV",
                        data=csv,
                        file_name=f"{st.session_state.symbol}_raw_data.csv",
                        mime="text/csv"
                    )
                else:
...
if submit_button and st.session_state.symbol and st.session_state.start_date and st.session_state.end_date:
    try:
        # Fetch stock data using yfinance
        ...
        # Check if data is returned
        if not df.empty:
            st.success(f"✅ Analysis complete for {st.session_state.symbol.upper()} from {st.session_state.start_date} to {st.session_state.end_date}")
            ...
            # --- Metric Comparisons ---
            selected_metrics = ["Open", "High", "Low", "Close", "Volume"] if st.session_state.comparison == "All" else [st.session_state.comparison]
            ...
            st.download_button(
                ...
                data=csv,
                file_name=f"{st.session_state.symbol}_analyzed_data.csv",
                mime="text/csv"
            )
        else:
            st.error("No data found for the given parameters.")
