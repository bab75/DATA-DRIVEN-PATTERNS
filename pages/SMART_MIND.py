...
comparison = st.radio(
    "Select metric to compare against previous day:",
...
if st.button("🚀 Analyze Pattern"):
    ...
    if df.empty:
        st.warning("No data returned. Check the symbol or selected date range.")
        st.stop()
    ...
    # Create lagged values for comparison
    ...
    # Recovery tag: did price bounce back from intraday dip?
    ...
    for col in cols_to_compare:
        ...
        if curr_col in df.columns and prev_col in df.columns:
            try:
                ...
                st.warning(f"Could not calculate change for {col}: {e}")
        else:
            st.warning(f"Missing columns for comparison: {col} or {prev_col}")
