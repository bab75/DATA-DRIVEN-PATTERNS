...
def load_data(primary_file, secondary_file):
    ...
    if primary_file:
        try:
            ...
            if missing_cols:
                st.error(f"Missing required columns in AAPL data: {', '.join(missing_cols)}. Please ensure the file contains all necessary columns.")
            ...
            if aapl_df.empty:
                st.error("AAPL data is empty after removing rows with null values in critical columns (date, open, high, low, close, volume).")
                return pd.DataFrame(), pd.DataFrame()
...
try:
    ...
    st.error(f"Error plotting candlestick chart: {str(e)}. Please ensure OHLC and volume columns contain valid numeric data.")
    st.stop()

if "Bollinger Bands" in show_indicators and 'std_dev' in aapl_df.columns:
